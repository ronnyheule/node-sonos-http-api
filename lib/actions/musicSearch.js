...
// rh: const musicTypes = ['album','song','station','load','playlist'];
const musicTypes = ['album','song','station','load','loadifneeded','playlist','listall','metadata',
                    'playsongfromhash','playalbumfromhash','queuesongfromhash'];
...
function doSearch(service, type, term)
{
  ...
  // rh: if (term.indexOf(':') > -1) {
  if (type != 'metadata' && !type.endsWith('hash') && term.indexOf(':') > -1) {  
    var newTerm = '';
...
// rh: begin new  
  if ((service == 'library') && (type == 'playalbumfromhash')) {
    // Already sorted by albumTrackNumber
...
//  }
  ...
  if (service == 'library') {
    if ((type == 'load') || libraryDef.nolib()) {
      return libraryDef.load(player, (type == 'load'));
    } else if (type == 'loadifneeded') {
      return Promise.resolve('Library already loaded!');
    } else if (type == 'listall') {
      return libraryDef.listall();
    }
  }
//rh: end
...
*/
              ...
              if (type == 'metadata') {
                let track = tracks.queueTracks[0];
                console.log(type + ' ' + JSON.stringify(track));
                return Promise.resolve(track);
              } else if (type.endsWith('hash')) {
                let track = tracks.queueTracks[0];
                let uri = track.uri;
                let metadata = track.metadata;
                if (type == 'playsongfromhash') {
                  // Clear the queue then play the song immediately
                  return player.coordinator.clearQueue()
                    .then(() => player.coordinator.addURIToQueue(uri, metadata))
                    .then(() => player.coordinator.setAVTransport(queueURI, ''))
                    .then(() => player.coordinator.play());
                } else { // (type == 'playalbumfromhash')
                  // Don't clear the queue first, just add to it
                  return player.coordinator.getQueue(0, 1)
                    .then((queue) => {
                      empty = (queue.length == 0);
                    })
                    ...
                    .then(() => {
                      if (empty) {
                        // Start playing if the queue was previously empty
